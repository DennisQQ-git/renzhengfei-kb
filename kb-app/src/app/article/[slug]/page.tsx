...
import { getDocument, getIndexData } from '@/lib/data'
...
export async function generateStaticParams() {
  ...
  index.documents.forEach(d => { if (d.slug) slugs.add(d.slug) })
  index.topics.forEach(t => { if (t.slug) slugs.add(t.slug) })
...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const doc = getDocument(slug)
  const index = getIndexData()
  const related = getRelated(doc, index.documents)
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      ...
      <Reveal>
        <Link
          href={doc.metadata?.bookSlug ? `/article/${doc.metadata.bookSlug}` : doc.year ? `/year/${doc.year}` : '/'}
        ...
        >
          ...
          {doc.metadata?.bookSlug
            ? `返回《${((doc.metadata as any).book || '').replace(/[《》]/g, '')}》`
            : doc.year ? `返回 ${doc.year} 年` : '返回首页'}
        </Link>
      </Reveal>
        ...
        <article>
          ...
          {chapterNav && (
            <div className="flex items-center justify-between mt-4 pt-3 border-t border-cream-100">
              <div>
                {chapterNav.prevSlug ? (
                  <Link href={`/article/${chapterNav.prevSlug}`} className="text-xs text-ink-400 hover:text-gold-600 transition-colors">
                    ← 上一章
                  </Link>
                ) : <span />}
              </div>
              <Link href={`/article/${chapterNav.bookSlug}`} className="text-xs text-gold-600 hover:text-gold-500 transition-colors font-medium">
                返回目录
              </Link>
              <div>
                {chapterNav.nextSlug ? (
                  <Link href={`/article/${chapterNav.nextSlug}`} className="text-xs text-ink-400 hover:text-gold-600 transition-colors">
                    下一章 →
                  </Link>
                ) : <span />}
              </div>
      ...
      {chapterNav && (
        <Reveal delay={2}>
          <div className="flex items-center justify-between border-t border-cream-200 pt-5">
            <div>
              {chapterNav.prevSlug ? (
                <Link href={`/article/${chapterNav.prevSlug}`} className="btn-secondary text-sm inline-flex items-center gap-1">
                  ← 上一章
                </Link>
              ) : <span />}
            </div>
            <Link href={`/article/${chapterNav.bookSlug}`} className="text-xs text-gold-600 hover:text-gold-500 transition-colors">
              返回目录
            </Link>
            <div>
              {chapterNav.nextSlug ? (
                <Link href={`/article/${chapterNav.nextSlug}`} className="btn-secondary text-sm inline-flex items-center gap-1">
                  下一章 →
                </Link>
              ) : <span />}
            </div>
      ...
      {!chapterNav && (
        <Reveal delay={3}>
          <div className="border-t border-cream-200 pt-6 mt-12">
            <Link href={doc.year ? `/year/${doc.year}` : '/'} className="btn-secondary text-sm">
